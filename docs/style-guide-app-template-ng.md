...
* Execute `tns create` CLI command to create an app from [the seed template](https://github.com/NativeScript/nativescript-app-templates/tree/master/packages/template-blank-ng) to [work-folder]\template-my-blank-ng:
```
tns create template-my-blank-ng --template tns-template-blank-ng
```
Now you can develop / deploy / debug your app template from [work-folder]\template-my-blank-ng, then you can commit changes to a git repo of your choice (e.g http://github.com/your-organzation/template-my-blank-ng).
...
Note this is not the actual root package.json of the generated mobile app – it is only used by the `tns create` CLI command upon app creation. Do not expect that everything you place in your package.json will be transferred to the actual package.json file. You can use the mechanism implemented by the [after-createProject hook](https://github.com/NativeScript/nativescript-app-templates/blob/master/packages/template-blank-ng/hooks/after-createProject/after-createProject.js) to generate / move settings files to the root folder of the generated app.
...
* Do publish your app template to npm (https://www.npmjs.com/) using **tns-template-[custom-template-name-goes-here]-ng** format for the npm package name.
